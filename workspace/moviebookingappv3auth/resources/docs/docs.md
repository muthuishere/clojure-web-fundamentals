...
The `home-routes` handler in the `moviebookingappv3auth.routes.home` namespace
...
The `home-page` function will in turn call the `moviebookingappv3auth.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `moviebookingappv3auth.handler` namespace:
...
Request middleware functions are located under the `moviebookingappv3auth.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `moviebookingappv3auth.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `moviebookingappv3auth.db.core` in some other namespace.
