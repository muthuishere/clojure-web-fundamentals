...
The `home-routes` handler in the `moviesappv1.routes.home` namespace
...
The `home-page` function will in turn call the `moviesappv1.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `moviesappv1.handler` namespace:
...
Request middleware functions are located under the `moviesappv1.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `moviesappv1.dev-middleware` namespace found in
